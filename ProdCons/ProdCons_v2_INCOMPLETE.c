...
///////////////////////////////////////////
TQueue          g_Queue;
pthread_mutex_t mtxProducer;
pthread_mutex_t mtxConsumer;
...
void *TaskConsumer(void *arg)
{
    ...
    while(1)
    {
        pthread_mutex_lock(&mtxConsumer); // lock other consumers
        ...
        pthread_mutex_unlock(&mtxProducer);
//        usleep(10000);
    }
}
...
void *TaskProducer(void *arg)
{
    ...
    while(1)
    {
        pthread_mutex_lock(&mtxProducer); // lock other producers
...
int main(){
    ...
    pthread_mutex_init(&mtxProducer, NULL);
    pthread_mutex_init(&mtxConsumer, NULL);

    // garante inicio dos consumidores
    pthread_mutex_lock(&mtxConsumer);

    for (int j = 0; j < PRODUCERS; j++)
        pthread_create(&producers[j], NULL, TaskProducer, (int*)nThreadID++);

    for (int i = 0; i < CONSUMERS; i++) {
        pthread_create(&consumers[i], NULL, TaskConsumer, (int*)nThreadID++);
    }
